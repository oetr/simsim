...
;; Stack
(define (get-sp)
  (ior (&<< (sram-get-byte #x5e) #b00000111 8)
       (sram-get-byte #x5d)))
(define (inc-sp (n -2))
  (define sp (+ (get-sp) n))
...
(define (fetch-and-decode)
  ...
  (cond [(= opcode #x95c8) ;;;;;;;;;;;; LPM
        ...
        [(and (= hb0 #b0000)  ;;;;;;;;;;;;;;;;;;;;;;;; MOVW
         ...
         (sram-set-byte Rd+ Rr+-val)
         (when debug? (fprintf OUT "MOVW R~a:R~a,R~a:R~a <- ~a ~a"  
                               Rd Rd+ Rr Rr+
                               (num->hexb Rr-val)
                               (num->hexb Rr+-val)))
         ]
        ...
        [(and (= hb0 #b0010)  ;;;;;;;;;;;;;;;;;;;; EOR
         ...
         (when debug? (fprintf OUT "EOR R~a,R~a ; R~a <- ~a xor ~a = ~a" 
                               Rd Rr Rd 
                               (num->hex Rd-val)
                               (num->hex Rr-val)
                               (num->hex R)))
        ...
        [(and (= hb0 #b1011)  ;;;;;;;;;;;;;;;;;;;; IN
         ...
         (when debug? (fprintf OUT "IN R~a,A(~a) ; R~a<-~a" 
                               Rd
                               (num->hex A)
                               Rd 
                               (num->hex A-val)))
        ...
        [(and (= hb0 #b0001)  ;;;;;;;;;;;;;;;;;;;; CP
         ...
         (if (one? (ior (& (n-bit-ref Rd-val 7)
                        ...
                        (& (bit-ref R 7)
                           (n-bit-ref Rd-val 7))))
             (sr-set-C) (sr-clear-C))
         (when debug? (fprintf OUT "CP R~a,R~a ; ~a-~a=~a"
        ...
        [(and (= hb0 #b1001)  ;;;;;;;;;;;;;;;;;;;; SBIW
         ...
         (if (one? (bitwise-xor (sr-get-N) (sr-get-V)))
             (sr-set-S) (sr-clear-S))
         (when debug? (fprintf OUT "SBIW R~a:R~a,K[~a] ; ~a-~a=~a"
                               Rd Rd+
                               (num->hex K) 
                               (num->hex Rd-val) 
                               (num->hex K) 
        ...
        [(and (= hb0 0)  ;;;;;;;;;;;;;;;;;;;; ADD without carry
         ...
         (if (one? (ior (& (bit-ref Rd-val 3)
             ...
             (sr-set-H)(sr-clear-H))
         (when debug? 
           (fprintf OUT "ADD R~a:R~a ; R~a <- ~a + ~a = ~a" 
                    Rd Rr Rd 
                    (num->hexb Rd-val) 
                    (num->hexb Rr-val)
                    (num->hexb R)))
        ...
        [(and (= hb0 1)  ;;;;;;;;;;;;;;;;;;;; ADC with carry
         ...
         (if (one? (ior (& (bit-ref Rd-val 3)
             ...
             (sr-set-H)(sr-clear-H))
         (when debug? 
           (fprintf OUT "ADC R~a:R~a ; R~a <- ~a + ~a + ~a = ~a" 
                    Rd Rr Rd 
                    (num->hexb Rd-val) 
                    (num->hexb Rr-val)
                    C
        ...
        [(and (= hb0 #b1001)  ;;;;;;;;;;;;;;;;;;;; LSR
         ...
         (if (one? (bitwise-xor (sr-get-N) (sr-get-V)))
             (sr-set-S) (sr-clear-S))
         (when debug? (fprintf OUT "LSR R~a[~a] ; <- ~a" 
                               Rd 
                               (num->hex Rd-val)
                               (num->hex R)))
         ]
